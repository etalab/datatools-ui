...
export default class FeedVersionDetails extends Component {
  ...
  render () {
    ...
    const userCanManageFeed = user && user.permissions &&
                              user.permissions.hasFeedPermission(version.feedSource.organizationId,
                                      version.feedSource.projectId, version.feedSource.id, 'manage-feed')
    return (
      <ListGroupItem>
        <h4>
