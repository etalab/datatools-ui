...
export default class Auth0Manager {
  constructor (props) {
    ...
    this.webauth = new auth0.WebAuth({
        ...
        redirectUri: "http://localhost:9966",
        responseType: "id_token token",
        scope: "openid",
    })
    this.userInfo = promisify(this.webauth.client.userInfo, this.webauth.client);
  ...
  checkExistingLogin (props, callbac) {
      var access_token = this.getToken()
      var id_token = this.getIdToken()
      if (access_token) return this.loginFromToken(access_token, id_token, props)
      return null
  }

  loginFromToken (token, id_token, props) {
    return this.userInfo(token)
       .then(prof => {
            return constructUserObj(token, id_token, prof)
    });
  }
